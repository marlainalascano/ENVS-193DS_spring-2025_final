...
|--------------|--------------|--------------|-------------------------------|
| 1 | No | No | Null model: Intercept only (no predictors) |
| 2 | Yes | Yes | Saturated model: Includes both season and edge distance |
| 3 | Yes | No | Season-only model: Includes only season as predictor |
| 4 | No | Yes | Distance-only model: Includes only edge distance as predictor |

e\)

```{r}

data <- read_csv("data/occdist.csv") %>%
  rename(edge_distance = `edge distance`)%>% #cleaning data
  mutate(season = as.factor(season))

model1 <- glm(sp ~ 1, data = data, family = "binomial") #intercept only null model

model2 <- glm(sp ~ season + edge_distance, data = data, family = "binomial") #sason + edge saturated model
